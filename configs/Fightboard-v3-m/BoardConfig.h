/*
 * SPDX-License-Identifier: MIT
 * SPDX-FileCopyrightText: Copyright (c) 2024 OpenStickCommunity (gp2040-ce.info)
 */

#ifndef PICO_BOARD_CONFIG_H_
#define PICO_BOARD_CONFIG_H_

#include "enums.pb.h"
#include "class/hid/hid.h"

#define BOARD_CONFIG_LABEL "fightboard-v3-m"
#define BOARD_SVG

#define BOARD_EXTRA_PINS {17, 18, 19, 20, 21, 22, 23, 24, 25}

// Boot-hold pin-based input mode overrides
#define DEFAULT_INPUT_MODE_XINPUT_PIN 9   // B1 (GP9)
#define DEFAULT_INPUT_MODE_SWITCH_PIN 12  // B2 (GP12)
#define DEFAULT_INPUT_MODE_PS3_PIN 15     // B3 (GP15)
#define DEFAULT_INPUT_MODE_PS4_PIN 28     // B4 (GP28)
#define DEFAULT_INPUT_MODE_PS5_PIN 27     // R1 (GP27)
#define DEFAULT_INPUT_MODE_KEYBOARD_PIN 26 // L1 (GP26)

// Main pin mapping Configuration
//                                                  // GP2040 | Xinput | Switch  | PS3/4/5  | Dinput | Arcade |
#define GPIO_PIN_00 GpioAction::BUTTON_PRESS_UP     // UP     | UP     | UP      | UP       | UP     | UP     |
#define KEYBOARD_KEYCODE_GP00 HID_KEY_ARROW_UP
#define GPIO_PIN_02 GpioAction::BUTTON_PRESS_DOWN   // DOWN   | DOWN   | DOWN    | DOWN     | DOWN   | DOWN   |
#define KEYBOARD_KEYCODE_GP02 HID_KEY_ARROW_DOWN
#define GPIO_PIN_03 GpioAction::BUTTON_PRESS_RIGHT  // RIGHT  | RIGHT  | RIGHT   | RIGHT    | RIGHT  | RIGHT  |
#define KEYBOARD_KEYCODE_GP03 HID_KEY_ARROW_RIGHT
#define GPIO_PIN_01 GpioAction::BUTTON_PRESS_LEFT   // LEFT   | LEFT   | LEFT    | LEFT     | LEFT   | LEFT   |
#define KEYBOARD_KEYCODE_GP01 HID_KEY_ARROW_LEFT
#define GPIO_PIN_09 GpioAction::BUTTON_PRESS_B1     // B1     | A      | B       | Cross    | 2      | K1     |
#define KEYBOARD_KEYCODE_GP09 HID_KEY_SHIFT_LEFT
#define GPIO_PIN_12 GpioAction::BUTTON_PRESS_B2     // B2     | B      | A       | Circle   | 3      | K2     |
#define KEYBOARD_KEYCODE_GP12 HID_KEY_Z
#define GPIO_PIN_13 GpioAction::BUTTON_PRESS_R2     // R2     | RT     | ZR      | R2       | 8      | K3     |
#define KEYBOARD_KEYCODE_GP13 HID_KEY_X
#define GPIO_PIN_14 GpioAction::BUTTON_PRESS_L2     // L2     | LT     | ZL      | L2       | 7      | K4     |
#define KEYBOARD_KEYCODE_GP14 HID_KEY_V
#define GPIO_PIN_15 GpioAction::BUTTON_PRESS_B3     // B3     | X      | Y       | Square   | 1      | P1     |
#define KEYBOARD_KEYCODE_GP15 HID_KEY_CONTROL_LEFT
#define GPIO_PIN_28 GpioAction::BUTTON_PRESS_B4     // B4     | Y      | X       | Triangle | 4      | P2     |
#define KEYBOARD_KEYCODE_GP28 HID_KEY_ALT_LEFT
#define GPIO_PIN_27 GpioAction::BUTTON_PRESS_R1     // R1     | RB     | R       | R1       | 6      | P3     |
#define KEYBOARD_KEYCODE_GP27 HID_KEY_SPACE
#define GPIO_PIN_26 GpioAction::BUTTON_PRESS_L1     // L1     | LB     | L       | L1       | 5      | P4     |
#define KEYBOARD_KEYCODE_GP26 HID_KEY_C
#define GPIO_PIN_07 GpioAction::BUTTON_PRESS_S1     // S1     | Back   | Minus   | Select   | 9      | Coin   |
#define KEYBOARD_KEYCODE_GP07 HID_KEY_5
#define GPIO_PIN_05 GpioAction::BUTTON_PRESS_S2     // S2     | Start  | Plus    | Start    | 10     | Start  |
#define KEYBOARD_KEYCODE_GP05 HID_KEY_1
#define GPIO_PIN_08 GpioAction::BUTTON_PRESS_L3     // L3     | LS     | LS      | L3       | 11     | LS     |
#define KEYBOARD_KEYCODE_GP08 HID_KEY_EQUAL
#define GPIO_PIN_04 GpioAction::BUTTON_PRESS_R3     // R3     | RS     | RS      | R3       | 12     | RS     |
#define KEYBOARD_KEYCODE_GP04 HID_KEY_MINUS
#define GPIO_PIN_06 GpioAction::BUTTON_PRESS_A1     // A1     | Guide  | Home    | PS       | 13     | ~      |
#define KEYBOARD_KEYCODE_GP06 HID_KEY_9

// Set physical pin to open webconfig
#define PIN_WEBCONFIG 5

// Hold this pin + PIN_WEBCONFIG at boot to show the configuration button in webconfig
#define PIN_WEBCONFIG_ADVANCED 7

// Setting GPIO pins to assigned by add-on
//
#define GPIO_PIN_10 GpioAction::ASSIGNED_TO_ADDON
#define GPIO_PIN_11 GpioAction::ASSIGNED_TO_ADDON
#define GPIO_PIN_29 GpioAction::ASSIGNED_TO_ADDON

// Hotkeys
#define HOTKEY_08_BUTTONS_MASK 3072  // L3+R3
#define HOTKEY_08_DPAD_MASK 4        // Left
#define HOTKEY_08_AUX_MASK 0
#define HOTKEY_08_ACTION 42          // HOTKEY_PREVIOUS_PROFILE

#define HOTKEY_09_BUTTONS_MASK 3072  // L3+R3
#define HOTKEY_09_DPAD_MASK 8        // Right
#define HOTKEY_09_AUX_MASK 0
#define HOTKEY_09_ACTION 35          // HOTKEY_NEXT_PROFILE

#define HOTKEY_10_USE_PIN_TRIGGER 1
#define HOTKEY_10_PIN_TRIGGER_MASK ((1 << 4) | (1 << 8) | (1 << 9))
#define HOTKEY_10_ACTION 50  // menu toggle

// Per-key LEDs
#define BOARD_LEDS_PIN 29
#define LED_FORMAT LED_FORMAT_GRB
#define LEDS_PER_PIXEL 1

// Pin → LED strip index mapping
#define BOARD_LED_INDEX_GP00  11
#define BOARD_LED_INDEX_GP01  10
#define BOARD_LED_INDEX_GP02  9
#define BOARD_LED_INDEX_GP03  8
#define BOARD_LED_INDEX_GP09  7
#define BOARD_LED_INDEX_GP12  6
#define BOARD_LED_INDEX_GP13  5
#define BOARD_LED_INDEX_GP14  4
#define BOARD_LED_INDEX_GP15  0
#define BOARD_LED_INDEX_GP26  3
#define BOARD_LED_INDEX_GP27  2
#define BOARD_LED_INDEX_GP28  1

// Spatial grid for ripple animation
#define BOARD_LED_POSITION_COLS 7
#define BOARD_LED_POSITIONS \
    {  3,  2,  1,  0, -1, 11, -1 }, \
    {  4,  5,  6,  7,  8,  9, 10 }

// Onboard WS2812 RGB LED (GPIO16) - shows the active input mode as a
// color. Runs alongside the per-switch LEDs above (BOARD_LEDS_PIN),
// each on its own PIO state machine.
// NOTE: this onboard chip is wired RGB, unlike the GRB per-switch
// chain above (LED_FORMAT) - the color order was swapped.
#define BOARD_LEDS_RGB_ENABLED 1
#define BOARD_LEDS_RGB_PIN 16
#define BOARD_LEDS_RGB_FORMAT LED_FORMAT_RGB
#define BOARD_LEDS_RGB_BRIGHTNESS 16
#define BOARD_LEDS_RGB_COLOR_PS4 0xFFFF00      // yellow
#define BOARD_LEDS_RGB_COLOR_PS5 0xFF00FF      // purple
#define BOARD_LEDS_RGB_COLOR_KEYBOARD 0xFF8000 // orange

// Boot-hold pin-based input mode overrides
#define DEFAULT_INPUT_MODE_XINPUT_PIN 9    // B1 (GP9)
#define DEFAULT_INPUT_MODE_SWITCH_PIN 12   // B2 (GP12)
#define DEFAULT_INPUT_MODE_PS3_PIN 15      // B3 (GP15)
#define DEFAULT_INPUT_MODE_PS4_PIN 28      // B4 (GP28)
#define DEFAULT_INPUT_MODE_PS5_PIN 27      // R1 (GP27)
#define DEFAULT_INPUT_MODE_KEYBOARD_PIN 26 // L1 (GP26)

#define HAS_I2C_DISPLAY 1
#define I2C1_ENABLED 1
#define I2C1_PIN_SDA 10
#define I2C1_PIN_SCL 11

#define BUTTON_LAYOUT BUTTON_LAYOUT_BOARD_DEFINED_A
#define BUTTON_LAYOUT_RIGHT BUTTON_LAYOUT_BOARD_DEFINED_B

// Start x pos at 0
#define DEFAULT_BOARD_LAYOUT_A {\
    {GP_ELEMENT_PIN_BUTTON, { 60,  25, 8, 8, 1, 1, 15, GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 60,  45, 8, 8, 1, 1, 9,  GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 40,  15, 8, 8, 1, 1, 28, GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 40,  35, 8, 8, 1, 1, 12, GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 20,  15, 8, 8, 1, 1, 27, GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 20,  35, 8, 8, 1, 1, 13, GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 0,  15, 8, 8, 1, 1, 26, GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 0,  35, 8, 8, 1, 1, 14, GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 0,  50, 4, 4, 1, 1, 8,  GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 10,  50, 4, 4, 1, 1, 7,  GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 20,  50, 4, 4, 1, 1, 6,  GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 30,  50, 4, 4, 1, 1, 5,  GP_SHAPE_ELLIPSE}},\
    {GP_ELEMENT_PIN_BUTTON, { 40,  50, 4, 4, 1, 1, 4,  GP_SHAPE_ELLIPSE}}\
}

// End x pos at 130; 22, 0, 19, 38 -> -16, -38, -19, 0 -> 114, 92, 111, 130
// y: 23, 39, 42, 45
#define DEFAULT_BOARD_LAYOUT_B {\
	{GP_ELEMENT_PIN_BUTTON, {108, 23, 8, 8, 1, 1, 0, GP_SHAPE_ELLIPSE}},\
	{GP_ELEMENT_PIN_BUTTON, {92, 45, 8, 8, 1, 1, 1, GP_SHAPE_ELLIPSE}},\
	{GP_ELEMENT_PIN_BUTTON, {111, 42, 8, 8, 1, 1, 2, GP_SHAPE_ELLIPSE}},\
	{GP_ELEMENT_PIN_BUTTON, {130, 39, 8, 8, 1, 1, 3, GP_SHAPE_ELLIPSE}},\
}

#define LEDS_BASE_ANIMATION_INDEX 3
#define LEDS_THEME_INDEX 19
#define LEDS_BUTTON_COLOR_INDEX 0
#define LEDS_BRIGHTNESS 40

#define INPUT_HISTORY_ENABLED 1
#define INPUT_HISTORY_TIMEOUT 3
#define DISPLAY_SAVER_TIMEOUT 60000
#define DISPLAY_SAVER_MODE DISPLAY_SAVER_STARS

#define SPLASH_MODE SPLASH_MODE_STATIC
#define SPLASH_DURATION 3000


// Mini menu navigation pins (compile-time, not remappable)
#define PIN_MENU_UP     0   // matches BUTTON_PRESS_UP
#define PIN_MENU_DOWN   2   // matches BUTTON_PRESS_DOWN
#define PIN_MENU_LEFT   1   // matches BUTTON_PRESS_LEFT
#define PIN_MENU_RIGHT  3   // matches BUTTON_PRESS_RIGHT
#define PIN_MENU_SELECT 9   // matches BUTTON_PRESS_B1
#define PIN_MENU_BACK   12  // matches BUTTON_PRESS_B2
#endif

